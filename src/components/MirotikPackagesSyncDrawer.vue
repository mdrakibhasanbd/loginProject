<template>
    <div class="drawer drawer-end sm:overflow-auto md:overflow-hidden">
      ...
      <div class="drawer-content bg-red-700  my-2 px-4 py-4  border rounded-md ">
        <h2 class=" font-semibold">Details for Row ID: {{ syncPackageDataLength }}{{ shouldShowDetails ? '/' + rowData.length : '' }}</h2>
      </div>
      <slot></slot>
  ...
  export default {
    ...
    computed: {
      ...
      shouldShowDetails() {
        return this.syncPackageDataLength > 0 && this.rowData.length > 0;
      },
    },
    methods: {

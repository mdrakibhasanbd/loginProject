        ...
        <TableBodyLayout>
            ...
            <!-- <div class=" bg-red-700 my-2 px-4 py-4 border rounded-md">
                <h2 class="font-semibold">
                    Details for Row ID: {{ mikrotikDataLength }}
                </h2>
            </div> -->
            <MikrotikDrawerVue :rowData="selectedRows" :itemsPerPage="itemsPerPage" @send-selected-rows="sendSelectedRowsDrawer">
                        ...
                        <tr class="uppercase bg-gray-300 context-row" @contextmenu.prevent="showContextMenuAt($event, -1)">
                            ...
                            <th v-if="mikrotikPort">
                                <span @click="sortTable('mikrotikPort')">
                                    ...
                                    <span v-else-if="isSortedDescending('mikrotikPort')"
                                        class="sort-icon inline-block ml-1">
                                        <SortedDescendingSvg />
                                    </span>
                                </span>
                ...
                <div class="flex flex-row gap-2 md:my-10  sm:mt-10">
...
import MikrotikDrawerVue from "./MikrotikDrawer.vue";
...
export default {
    ...
    computed: {
        // Reference the store and mikrotikData directly
        networkDevicesStore() {
        ...
        filteredData() {
            ...
            const filteredRows = this.networkDevicesStore.mikrotikData.filter(
                (row) => {
    ...
    methods: {
        async sendSelectedRows() {
            try {
                const dataToSend = this.selectedRows.map((rowId) => {
                    const row = this.networkDevicesStore.mikrotikData.find(
                        (item) => item.id === rowId
                    );
                    return {
                        id: row.id,
        ...
        async editDevice(deviceId) {
            ...
            const device = this.networkDevicesStore.mikrotikData.find(
        ...
        async DevicesData() {
            try {
                await this.networkDevicesStore.fetchMikrotikData();
            } catch (error) {
    ...
    mounted() {
        ...
        console.log(this.networkDevicesStore.mikrotikData);
